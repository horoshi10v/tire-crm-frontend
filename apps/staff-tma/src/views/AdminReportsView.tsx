    ...
    <section className="min-h-full space-y-4 bg-gray-950 p-4 text-white">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-gray-800 bg-gray-950 p-4">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
              <div className="rounded-xl border border-blue-700/30 bg-blue-500/10 p-3"><p className="text-xs uppercase tracking-wide text-blue-200/80">Нові</p><p className="mt-2 text-xl font-bold text-white">{dashboardStats.newOrders}</p></div>
              <div className="rounded-xl border border-amber-700/30 bg-amber-500/10 p-3"><p className="text-xs uppercase tracking-wide text-amber-200/80">Передплата</p><p className="mt-2 text-xl font-bold text-white">{dashboardStats.inProgressOrders}</p></div>
              <div className="rounded-xl border border-emerald-700/30 bg-emerald-500/10 p-3"><p className="text-xs uppercase tracking-wide text-emerald-200/80">Завершені</p><p className="mt-2 text-xl font-bold text-white">{dashboardStats.doneOrders}</p></div>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-xl border border-amber-700/30 bg-amber-500/10 p-3"><p className="text-xs uppercase tracking-wide text-amber-200/80">Офлайн</p><p className="mt-2 text-xl font-bold text-white">{dashboardStats.offlineOrders}</p></div>
              <div className="rounded-xl border border-blue-700/30 bg-blue-500/10 p-3"><p className="text-xs uppercase tracking-wide text-blue-200/80">Онлайн</p><p className="mt-2 text-xl font-bold text-white">{dashboardStats.onlineOrders}</p></div>
            </div>
            <div className="mt-3 rounded-xl border border-gray-800 bg-gray-900 p-3 text-sm">
              ...
              <p className="mt-2 text-xl font-bold text-white">{dashboardStats.doneOrders > 0 ? formatNumber((dashboardStats.doneOfflineOrders / (dashboardStats.doneOfflineOrders + dashboardStats.doneOnlineOrders || 1)) * 100) : '0'}%</p>
              <p className="mt-1 text-xs text-gray-500">Від завершених продажів: {dashboardStats.doneOfflineOrders} офлайн / {dashboardStats.doneOnlineOrders} онлайн</p>
            </div>
          </div>
      ...
      <article className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={handleExportPnL} disabled={exportPnlMutation.isPending} className="rounded-lg border border-blue-700/60 bg-blue-900/30 px-3 py-2 text-sm font-semibold text-blue-200 transition hover:bg-blue-900/45 disabled:cursor-not-allowed disabled:opacity-50">{exportPnlMutation.isPending ? 'Експорт...' : 'Експорт P&L у Sheets'}</button>
        </div>

        <div className="grid grid-cols-1 gap-2 sm:grid-cols-4">
          <label className="space-y-1"><span className="text-sm text-gray-300">Дата початку</span><input type="date" value={pnlFilters.startDate} onChange={(event) => setPnlFilters((prev) => ({ ...prev, startDate: event.target.value }))} className="w-full rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" /></label>
          <label className="space-y-1"><span className="text-sm text-gray-300">Дата завершення</span><input type="date" value={pnlFilters.endDate} onChange={(event) => setPnlFilters((prev) => ({ ...prev, endDate: event.target.value }))} className="w-full rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" /></label>
          <label className="space-y-1"><span className="text-sm text-gray-300">Склад</span><select value={pnlFilters.warehouseId} onChange={(event) => setPnlFilters((prev) => ({ ...prev, warehouseId: event.target.value }))} className="w-full rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500"><option value="">Усі склади</option>{warehouses.map((warehouse) => (<option key={warehouse.id} value={warehouse.id}>{warehouse.name} ({warehouse.location})</option>))}</select></label>
          <label className="space-y-1"><span className="text-sm text-gray-300">Канал</span><select value={pnlFilters.channel} onChange={(event) => setPnlFilters((prev) => ({ ...prev, channel: event.target.value as '' | 'ONLINE' | 'OFFLINE' }))} className="w-full rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500"><option value="">Усі канали</option><option value="ONLINE">Онлайн</option><option value="OFFLINE">Офлайн</option></select></label>
        </div>

        <div className="mt-3 flex gap-2">
          <button type="button" onClick={applyPnLFilters} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-emerald-500">Застосувати фільтр</button>
          <button type="button" onClick={resetPnLFilters} className="rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm font-semibold text-gray-200 transition hover:bg-gray-700">Скинути</button>
        </div>
        ...
        {!isPnLLoading && !isPnLError && pnlReport ? (
          <div className="mt-3 space-y-3">
            <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
              <div className="rounded-lg border border-gray-800 bg-gray-950 p-2"><p className="text-xs text-gray-500">Дохід</p><p className="font-semibold text-white">{formatNumber(pnlReport.total_revenue)}</p></div>
              <div className="rounded-lg border border-gray-800 bg-gray-950 p-2"><p className="text-xs text-gray-500">Собівартість</p><p className="font-semibold text-white">{formatNumber(pnlReport.total_cogs)}</p></div>
              <div className="rounded-lg border border-gray-800 bg-gray-950 p-2"><p className="text-xs text-gray-500">Прибуток</p><p className="font-semibold text-white">{formatNumber(pnlReport.total_profit)}</p></div>
              <div className="rounded-lg border border-gray-800 bg-gray-950 p-2"><p className="text-xs text-gray-500">Продано шт.</p><p className="font-semibold text-white">{formatNumber(pnlReport.total_items_sold)}</p></div>
            </div>

            <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-2">
              <div className="rounded-xl border border-amber-700/30 bg-amber-500/10 p-3"><p className="text-xs uppercase tracking-wide text-amber-200/80">Офлайн виторг</p><p className="mt-2 text-xl font-bold text-white">{formatNumber(offlinePnL?.revenue ?? 0)}</p><p className="mt-1 text-xs text-amber-100/70">Прибуток: {formatNumber(offlinePnL?.profit ?? 0)} • Продано: {formatNumber(offlinePnL?.items_sold ?? 0)}</p></div>
              <div className="rounded-xl border border-blue-700/30 bg-blue-500/10 p-3"><p className="text-xs uppercase tracking-wide text-blue-200/80">Онлайн виторг</p><p className="mt-2 text-xl font-bold text-white">{formatNumber(onlinePnL?.revenue ?? 0)}</p><p className="mt-1 text-xs text-blue-100/70">Прибуток: {formatNumber(onlinePnL?.profit ?? 0)} • Продано: {formatNumber(onlinePnL?.items_sold ?? 0)}</p></div>
            </div>
              ...
              <div className="space-y-2">
                {pnlReport.by_warehouse.map((row) => (
                  <div key={row.warehouse_name} className="rounded-lg border border-gray-800 bg-gray-900 p-2 text-sm">
                    ...
                    <p className="text-gray-300">Дохід {formatNumber(row.revenue)} • Собівартість {formatNumber(row.cogs)} • Прибуток {formatNumber(row.profit)}</p>
      ...
      <article className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={handleExportInventory} disabled={exportInventoryMutation.isPending} className="rounded-lg border border-blue-700/60 bg-blue-900/30 px-3 py-2 text-sm font-semibold text-blue-200 transition hover:bg-blue-900/45 disabled:cursor-not-allowed disabled:opacity-50">{exportInventoryMutation.isPending ? 'Експорт...' : 'Експорт складу'}</button>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          <input type="number" step="0.1" value={inventoryFilters.width} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, width: event.target.value }))} placeholder="Ширина" className="rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" />
          <input type="number" step="0.1" value={inventoryFilters.profile} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, profile: event.target.value }))} placeholder="Профіль" className="rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" />
          <input type="number" step="0.1" value={inventoryFilters.diameter} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, diameter: event.target.value }))} placeholder="Діаметр" className="rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" />
          <input type="number" step="0.01" value={inventoryFilters.sellPrice} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, sellPrice: event.target.value }))} placeholder="Ціна продажу" className="rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" />
          <input type="number" value={inventoryFilters.currentQuantity} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, currentQuantity: event.target.value }))} placeholder="Кількість" className="rounded-lg border border-gray-700 bg-gray-950 px-3 py-2 text-white outline-none focus:border-blue-500" />
        </div>

        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
          <label className="flex items-center gap-2 rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-sm text-gray-200"><input type="checkbox" checked={inventoryFilters.isRunFlat} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, isRunFlat: event.target.checked }))} />Run Flat</label>
          <label className="flex items-center gap-2 rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-sm text-gray-200"><input type="checkbox" checked={inventoryFilters.isSpiked} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, isSpiked: event.target.checked }))} />Шипи</label>
          <label className="flex items-center gap-2 rounded-lg border border-gray-800 bg-gray-950 px-3 py-2 text-sm text-gray-200"><input type="checkbox" checked={inventoryFilters.antiPuncture} onChange={(event) => setInventoryFilters((prev) => ({ ...prev, antiPuncture: event.target.checked }))} />Антипрокол</label>
        </div>

        {inventoryExportError ? <div className="mt-3 rounded-lg border border-red-800/60 bg-red-950/40 px-3 py-2 text-sm text-red-300">{inventoryExportError}</div> : null}
        {inventoryExportUrl ? <a href={inventoryExportUrl} target="_blank" rel="noreferrer" className="mt-3 block break-all text-sm text-blue-300 hover:text-blue-200">{inventoryExportUrl}</a> : null}
      </article>
    </section>
  );
}
